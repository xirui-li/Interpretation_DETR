...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, losses, focal_alpha=0.25):
        ...
        label_empty_weight = torch.ones(self.num_classes)
        label_empty_weight[0] = 0.1
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        # target_classes_o = torch.ones_like(target_classes_o) #for caltech
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        labels = 1 + (topk_indexes % (out_logits.shape[2]-1))
...
class PostProcess_caltech(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        scores = topk_values
        topk_boxes = topk_indexes // (out_logits.shape[2]-1)
        labels = 1 + (topk_indexes % (out_logits.shape[2]-1))#% (out_logits.shape[2]-1)),torch.zeros_like(topk_indexes) 
        boxes = box_ops.box_cxcywh_to_xyxy(out_bbox)
        boxes = torch.gather(boxes, 1, topk_boxes.unsqueeze(-1).repeat(1,1,4))
        ...
        for result in results:
            class_idx = torch.arange(0,100,device=scores.device)
